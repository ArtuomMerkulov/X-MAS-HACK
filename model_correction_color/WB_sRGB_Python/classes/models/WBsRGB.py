...
class WBsRGB:
  def __init__(self, gamut_mapping=2, upgraded=0):
    if upgraded == 1:
      ...
      self.features = np.load('classes/models/features.npy')  # encoded features
      self.mappingFuncs = np.load('classes/models/mappingFuncs.npy')  # correction funcs
      self.encoderWeights = np.load('classes/models/encoderWeights.npy')  # PCA matrix
      self.encoderBias = np.load('classes/models/encoderBias.npy')  # PCA bias
